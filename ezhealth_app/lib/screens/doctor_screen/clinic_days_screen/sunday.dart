...
class SundayScreen extends StatefulWidget {
  ...
  SundayScreen(this.doctorId);

  @override
  _SundayScreenState createState() => _SundayScreenState(doctorId);
}

class _SundayScreenState extends State<SundayScreen> {
  final String doctorId;
  _SundayScreenState(this.doctorId);
  ...
  sendData() async {
    ...
    var response = await http.put(Uri.parse(url), body: {
      ...
      "sunday_name": doctorId
    });

    if (response.statusCode == 200) {
      print(response.body);
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text("Sunday Clinic Uploaded")));
    } else {
      ScaffoldMessenger.of(context)
